...
Well_Num=2;
% Model_Num=20;
% Calculate the model number 2015-4-24 %
...
for model_id=1:Model_Num
    %     According to the model id, select the geometry 2015-4-23 %
    [Wells,Shot]=Set_Geometry(Well_Num,Azimuth(:,model_id));
    ...
    for i=1:4
        ...
        for j=1:RandMT_Num
            ...
            [PWavefield_xyz1,PWaveform_OriXYZ,PWaveform_XYZ1]=Generate_Pwave_WithQ(Sample_Int,Travel_T1,Direction_Cosin1,...
                SR_Vec_model1,MT,Rho,Vp,Ricker_Derivative,Qp,Layer_Num,ELayerTime_SR);
            %         Generate the S-wave field
            [SWavefiled_xyz1,SWaveform_OriXYZ,SWaveform_XYZ1]=Generate_Swave_WithQ(Sample_Int,Travel_T1,Direction_Cosin1,...
            ...
            [Attenuated_Energy(model_id,:)]=Cal_AttEnergy(Well_Num,PWaveform_OriXYZ,SWaveform_OriXYZ,PWaveform_XYZ1,SWaveform_XYZ1);
...
for model_id=1:Model_Num
    ...
    Plot_Tk(Tk_Orig_All,Tk_PS_All,3)
    MT_Name_Cur=char(MT_Name(i));
    Title=['Inversion Results - Model Id=',num2str(model_id)];
    title(Title,'FontSize',FontSize);
    %     set(f1,'PaperPositionMode','manual','PaperUnits','centimeters','PaperPosition',[0 0 9 7]);
    set(f1,'PaperPositionMode','manual','PaperUnits','point','PaperPosition',[0 0 600 500]);
...
% Plot_ErrorV3(Model_Num,AverError_AbsoTk_PS,ErrorVariance_AbsoTk_PS,3);
% Plot_ErrorV3(Model_Num,AverError_AbsoTk_PS,ErrorVariance_AbsoTk_PS,4);
%     
