function Plot_TkErr_VariQ(Q_Err,AverError_AbsoTk_PS,ErrorVariance_AbsoTk_PS,...
    AverError_AbsoTk_P,ErrorVariance_AbsoTk_P,AverError_AbsoTk_S,ErrorVariance_AbsoTk_S)
% % % % % % % % % % % % % 2015-4-23 % % % % % % % % % % % % % % % % % % 
...
% Plot  the average error and error variance of Tk for all the sources only using S wave %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
for i=1:4
    ...
    %2015-4-27 %
    %
    plot(Q_Err,AverError_AbsoTk_PS(i,:,1),'-or','LineWidth',LineWidth)
    plot(Q_Err,AverError_AbsoTk_PS(i,:,2),'-^r','LineWidth',LineWidth)
    plot(Q_Err,AverError_AbsoTk_P(i,:,1),'-ok','LineWidth',LineWidth)
    plot(Q_Err,AverError_AbsoTk_P(i,:,2),'-^k','LineWidth',LineWidth)
    plot(Q_Err,AverError_AbsoTk_S(i,:,1),'-og','LineWidth',LineWidth)
    plot(Q_Err,AverError_AbsoTk_S(i,:,2),'-^g','LineWidth',LineWidth)
    set(gca,'FontSize',FontSize,'XLim',[Q_Err(1) Q_Err(Model_Num)])
%     set(gca,'YLim',[-1.5 1.2])
    %     set(gca,'FontSize',FontSize,'YLim',[-1 1],'XLim',[0 2*pi])
    xlabel('Vari_Q','FontSize',FontSize);
    %}
...
for i=1:4
    ...
    %2015-4-24 %
    %
    plot(Q_Err,ErrorVariance_AbsoTk_PS(i,:,1),':or','LineWidth',LineWidth);
    plot(Q_Err,ErrorVariance_AbsoTk_PS(i,:,2),':^r','LineWidth',LineWidth);
    plot(Q_Err,ErrorVariance_AbsoTk_P(i,:,1),':ok','LineWidth',LineWidth);
    plot(Q_Err,ErrorVariance_AbsoTk_P(i,:,2),':^k','LineWidth',LineWidth);
    plot(Q_Err,ErrorVariance_AbsoTk_S(i,:,1),':og','LineWidth',LineWidth);
    plot(Q_Err,ErrorVariance_AbsoTk_S(i,:,2),':^g','LineWidth',LineWidth);
    set(gca,'FontSize',FontSize,'XLim',[Q_Err(1) Q_Err(Model_Num)])
