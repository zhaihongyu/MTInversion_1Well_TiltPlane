function [Vp,Vs,Rho,LayerNum_EverySR,ELayerTime_SR,Qp,Qs]=Set_Model_MultiRec(Receivers,Shot)
...
% [Well_Num,RecNum_1Well,Components]=size(Wells);
% Rec_Num=Well_Num*RecNum_1Well;
...
Wells=zeros(Rec_Num_1Well*Well_Num,Coms);
for i=1:Well_Num
    s=(i-1)*Rec_Num_1Well+1;
    e=i*Rec_Num_1Well;
    Wells(s:e,:)=Receivers(:,:,i);
end

% Calculate the attenuation time of different Q
Layer_Z=zeros(LayerNum+1,1);
for i=1:LayerNum+1
    Layer_Z(i)=Deep_Initial+(i-1)*Layer_Int;
end
Length=zeros(Rec_Num_ALL,1);
LayerNum_EverySR=zeros(Rec_Num_ALL,1);
Layers_ShotRec=cell(Rec_Num_ALL,1);
SR_Z=zeros(1,Rec_Num_ALL);
%     According the z coordinate of receiver, identify the Q index  in every layer
%{
for i=1:Well_Num
    for j=1:RecNum_1Well
        Length((i-1)*RecNum_1Well+j)=norm([Wells(i,j,1)-Shot(1),Wells(i,j,2)-Shot(2),Wells(i,j,3)-Shot(3)]);
        SR_Z((i-1)*RecNum_1Well+j)=Shot(3)-Wells(i,j,3);
        %     Calculate the layer number between the shot and receiver
        if SR_Z(i)>0
            Layers1=find(Layer_Z<Shot(3));
            Layers2=find(Layer_Z(Layers1)>Wells(i,j,3));
            Layers_ShotRec{(i-1)*RecNum_1Well+j}=sort(Layer_Z(Layers2),'descend');
            LayerNum_EverySR((i-1)*RecNum_1Well+j)=size(Layers2,1)+1;
        else
            Layers1=find(Layer_Z>Shot(3));
            Layers2=find(Layer_Z(Layers1)<Wells(i,3));
            Layers_ShotRec{(i-1)*RecNum_1Well+j}=sort(Layer_Z(Layers2),'ascend');
            LayerNum_EverySR((i-1)*RecNum_1Well+j)=size(Layers2,1)+1;
        end
    end
end
...
for i=1:Rec_Num_ALL
    ...
    if SR_Z(i)>0
        ...
        LayerNum_EverySR(i)=size(Layers2,1)+1;
    else
        Layers1=find(Layer_Z>Shot(3));
        Layers2=find(Layer_Z(Layers1)<Wells(i,3));
...
Layer_Num_Max=max(LayerNum_EverySR);
ELayerTime_SR=zeros(Rec_Num_ALL,Layer_Num_Max,2);
for i=1:Rec_Num_ALL
