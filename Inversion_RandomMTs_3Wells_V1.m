...
M(:,:,4)=M_DC;
%}
% Generate random seismic moment tensors 2015-6-17 %
...
M(:,:,4)=M_CLVD_Pos;
%} 
% RandMT_ValueOri=rand(6,RandMT_Num);
...
% Generate and plot the P&S-wave field of DC source 2015-6-8 % 
dB=0;
%{
dB=45;
...
%                 (PWaveform_XYZ1,SWaveform_XYZ1,Ricker_EffValue_Idx);
            ...
            [T_k_Orig]=MT_Transform(RandomMTs(:,j));
...
Plot_InvRes_RandMts_QErrs(Model_Num,RandMT_Num,Sour_Num,Q_Err,...
...
for model_id=1:Model_Num
    ...
    [TkInversionS_XY]=Tk_Transform(Tk_S_All);
    % % % % % % % % % % % % % % % % % % % % % % % %
    f2=figure;
...
for model_id=1:Model_Num
    ...
    Tk_All_New(2,:,1)=k_P_All(:,model_id,1)';
    % Transform the T-k parameters to x-y coordinates
    [TkOriginal_XY]=Tk_Transform(Tk_Orig_All);  
    [TkInversion_XY]=Tk_Transform(Tk_All_New);
...
%     plot(TkInversionPS_XY(1,:),TkInversionPS_XY(2,:),'.r','Markersize',Markersize,'LineWidth',LineWidth);
%     plot(TkInversionP_XY(1,:),TkInversionP_XY(2,:),'.r','Markersize',Markersize,'LineWidth',LineWidth);
    plot(TkInversion_XY(1,:),TkInversion_XY(2,:),'.r','Markersize',Markersize,'LineWidth',LineWidth);
...
% Plot the original moment tensors 
f1=figure;
