...
function [Receivers,Shot,Well_Num,Vp,Vs,Layer_Z,Plane_Function]=Set_Geometry_1Well_V1(Interface_Type)
...
switch Interface_Type
    ...
    case 3
        ...
        %Secondly, define the rotating angle of three axises
        Receivers_Rotate=ones(Rec_Num_1Well,3,Well_Num);
        Receivers_Radius=zeros(1,Rec_Num_1Well);
        
        XAxis_Rotate=0;
        YAxis_Rotate=0;
        ZAxis_Rotate=0;
        XRotate_Matrix=[1, 0, 0;0, cos(XAxis_Rotate),sin(XAxis_Rotate);0,-sin(XAxis_Rotate),cos(XAxis_Rotate)];
        YRotate_Matrix=[cos(YAxis_Rotate),0,-sin(YAxis_Rotate);0,1,0;sin(YAxis_Rotate),0,cos(YAxis_Rotate)];
        ZRotate_Matrix=[cos(ZAxis_Rotate),sin(ZAxis_Rotate),0;-sin(ZAxis_Rotate),cos(ZAxis_Rotate),0;0,0,1];
        %Rotate the well one by one
        for i=1:Well_Num
            Receivers_Rotate(:,:,i)=(XRotate_Matrix*YRotate_Matrix*ZRotate_Matrix*Receivers(:,:,i)')';
            Receivers_Radius=sqrt(Receivers_Rotate(:,1,i).^2+Receivers_Rotate(:,2,i).^2);
            Receivers(:,:,i)=Receivers_Rotate(:,:,i);
        end
        % Shot point coordinate 2015-5-30 %
        Shot=[0 0 Receivers_Rotate(Rec_Num_1Well/2,3,1)];
        ...
        Plane_Function(2)=0.0;
        Plane_Function(3)=-1;
        [Plane_Function,Layer_Z]=Plot_Model(Model_X,Model_Y,Receivers_Rotate,Shot,Plane_Function);
    otherwise
end
